...
# Override with: make build VERSION=0.2.0
...
KIND_CLUSTER ?= gastown-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name gastown-operator-builder
	$(CONTAINER_TOOL) buildx use gastown-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm gastown-operator-builder
	rm Dockerfile.cross
...
CONTROLLER_TOOLS_VERSION ?= v2.9.0
